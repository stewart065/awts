<?php
            ...
            $sql="SELECT `id`, `Arrival`, `Departure`, `FirstN`, `LastN`, `Email`, `Phone`, `Adults`, `Children`, `Roomid`, `price`, `qty`, `subtotal`, `amount`, `total` FROM `res` order by id desc limit 1";
            $res = $con->query($sql);
            $row = $res->fetch_assoc();
...
<body>
    <div class="container d-lg-flex"> 
        <div class="box-1 bg-light user" style="margin-left: 180px; margin-top: 30px;"> 
            ...
            <div class="carousel-inner"> 
                        ...
                        <img src="images/img-2.png" class="d-block w-100"> 
                    </div> 
                ...
                <button class="carousel-control-prev" type="button" data-bs-target="#my" data-bs-slide="prev">
                    ...
                    <div class="dis">
                        ...
                        <div> 
                            <p class="fas fa mb-3 pe-2">Number of People:</p><br>
                            <span class="dis footer my-3"> Adult: <?php echo $row['Adults']?> </span> <br>
                            <span class="dis footer my-3"> Children: <?php echo $row['Children']?> </span>
                        </div> 
                                
                        <div> 
                            <p class="fas fa mb-3 pe-2">Payment:</p> <br>
                            <span class="dis footer my-3" > Price:$ <?php echo $row['price']?> </span> <br>
                            <span class="dis footer my-3"> Quanity: <?php echo $row['qty']?> </span> <br>
                            <span class="dis footer my-3"> Subtotal:$ <?php echo  $row['subtotal']?> </span> <br>
                            <span class="dis footer my-3"> total:$ <?php echo  $row['total']?> </span> <br>
                            <span class="dis footer my-3"> Amount:$ <?php echo  $row['amount']?> </span> <br>
                            <span class='dis footer my-3'><?php $sql = "SELECT `subtotal`, `amount` FROM `res` order by id desc";
                                        $res = $con->query($sql);
                                        if (!$res) {
                                            die("Error: " . $con->error);
                                        }
                                        $row = $res->fetch_assoc();
                                        $total_cost = $row["amount"] - $row["subtotal"];
                                        $con->close();
                                        
                                                
                                                echo"Change:$ ".$total_cost; ?></span>
                        </div> 
                    </div>
            </div>
